...
def generate_live_chart(coin: str, timeframe: str = '4h', limit: int = 60,
                        exchange=None, position=None) -> bytes:
    """Generate chart from LIVE exchange data (includes current forming candle).

    Args:
        coin: e.g. 'AVAX'
        timeframe: '1h', '4h', '1d'
        limit: number of candles
        exchange: BybitClient instance (uses ccxt fetch_ohlcv)
        position: dict with {direction, entry_price} to mark on chart, or None

    Returns: PNG bytes
    """
    if not exchange:
        return generate_chart(coin, timeframe, limit)
    ...
    try:
        ...
        ohlcv = exchange._exchange.fetch_ohlcv(symbol, timeframe, limit=limit)
        if not ohlcv or len(ohlcv) < 20:
            return generate_chart(coin, timeframe, limit)
    except Exception as e:
        logger.debug(f"Live OHLCV {coin}: {e}")
        return generate_chart(coin, timeframe, limit)
    ...
    # MA20/MA50
    ma20 = np.convolve(closes_arr, np.ones(20)/20, mode='valid')
    ma50_len = min(50, len(closes_arr))
    ma50 = np.convolve(closes_arr, np.ones(ma50_len)/ma50_len, mode='valid')
    ...
    # Mark CURRENT candle (last one = still forming)
    ax1.axvline(dates[-1], color='#FFD700', linewidth=0.5, alpha=0.3, linestyle=':')

    # Mark our position entry if provided
    if position:
        entry = position.get('entry_price', 0)
        direction = position.get('direction', '')
        if entry > 0:
            color = '#00FF88' if direction == 'LONG' else '#FF4444'
            label = f"OUR {direction} @${entry:.4f}"
            ax1.axhline(entry, color=color, linewidth=1.2, linestyle='--', alpha=0.8, label=label)

    title_time = datetime.now(timezone.utc).strftime("%H:%M UTC")
    ax1.set_title(f'{coin}/USDT {timeframe.upper()} LIVE — {title_time}',
                  color='#E6EDF3', fontsize=14)
    ax1.legend(loc='upper left', fontsize=8, facecolor='#0D1117', edgecolor='#1C2333',
               labelcolor='#8B949E')
